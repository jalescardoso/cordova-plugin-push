...
```bash
cordova plugin add @jalescardoso/cordova-plugin-push
```
...
```bash
cordova plugin add github:jalescardoso/cordova-plugin-push
```
...
```xml
<plugin name="@jalescardoso/cordova-plugin-push" spec="3.0.0" />
```
...
```json
{
  "cordova": {
    "plugins": {
      "@jalescardoso/cordova-plugin-push": {
...
When some other packages include `cordova-google-play-services` as a dependency, such as is the case with the `cordova-admob` and `cordova-plugin-analytics` plugins, it is impossible to also add the `@jalescardoso/cordova-plugin-push`, for the following error will rise during the build process:
...
```log
Installing "@jalescardoso/cordova-plugin-push" for ios
Failed to install '@jalescardoso/cordova-plugin-push':Error: pod: Command failed with exit code 1
